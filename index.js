...
function customAssign(...objects) {
  ...
  if (typeof(objects) == "object") {
    let nob = Object.assign({}, ...objects)    
    for (const key in nob) {
      nob[key] = nob[key].toString()
    }
    return nob
  } else {
...
function customObjectFromEntries(entries) {
  ...
  if (Array.isArray(entries)) {
    let newarr =  entries.map((e) => typeof(e[0]) == 'string' ? e : [e[0].toString(), e[0].toString()])
...
function filterObjectsByKey(arr, key) {
  ...
  if (Array.isArray(arr)) {
    return arr.filter((e) => e.hasOwnProperty(key))   
  } else {
    return []
  }
}
...
function getObjectValuesSum(obj) {
  ...
  // Повертаємо суму
  if (typeof(obj) == "object") {
    return Object.values(obj).reduce((a, e) => a + e);
  } else {
    return 0
  } 
}
  ...
  if (Array.isArray(arr)) {
    ...
    for (let i = 0; i < arr.length; i++) {
      ...
      if (!obj.hasOwnProperty(k)) {
        ...
        console.log(`У масиві є дубльований ключ: ${k}`);
      }  
